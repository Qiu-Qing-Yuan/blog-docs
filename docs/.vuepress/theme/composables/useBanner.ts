const modules = import.meta.glob("../../public/images/index/*.jpg", { eager: true });
...
export const useBanner = () => {
    ...
    /**开始自动轮播 */
    const startAutoplay = (images: string[]) => {
        if (!bannerOptions.isBgImagetrigger || images.length <= 1) return;
        timer = setInterval(() => {
            currentIndex.value = (currentIndex.value + 1) % images.length;
        }, bannerOptions.bgImageSec);
    };
    ...
    onMounted(() => {
        ...
        watch(isMobile, (newVal) => {
            bannerOptions.bgImageStyle = newVal ? { color: '#fff' } : bgImageStyle;
        });
    });
    ...
    return { getRandomImage, bannerOptions, currentIndex, startAutoplay, stopAutoplay };
};
